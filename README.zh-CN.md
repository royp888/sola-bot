...
- 积分排行榜与积分流水
- **群管功能**：封禁、禁言、踢人、警告、批量删消息、管理员权限提升/降级/头衔设置、举报、清理注销账号、关键词过滤、垃圾评分、链接域名白名单/黑名单
...
- **群规管理**：`/setrules` 设置群规、`/clearrules` 清除群规、`/rules` 查看群规
- **sed 文本修正**：回复消息并发送 `s/旧词/新词`，Bot 自动重发修正版本
- 抽奖系统，支持按钮参与和口令参与（`poll_answer` 实时接收）
...
- ✅ 群规管理（/setrules /clearrules /rules）
- ✅ 批量删消息（/purge /del）
- ✅ 管理员权限提升/降级/头衔（/promote /demote /set_title）
- ✅ 举报 + 清理注销账号（/report /ban_ghosts）
- ✅ sed 行内文本修正
- ✅ 细粒度管理员权限（verify / keyword / points 独立授权）
- [ ] 补充更完整的仓库截图和演示材料
- [ ] 多语言 i18n 支持
- [ ] 增加更多 Bot 流程的自动化测试覆盖
- [ ] 频道消息自动同步到群
